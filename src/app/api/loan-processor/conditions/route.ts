...
export async function POST(req: NextRequest) {
  ...
  // Optional pin to a specific person on the loan. Validate against the
  // loan's actual staff slots — silently null on mismatch (defense in depth;
  // the UI only exposes valid choices).
  ...
  try {
    await adminClient.from('loan_events').insert({
      ...
      description: `Loan processor ${lp.full_name} added condition (${assigned_to === 'loan_officer' ? 'Loan Officer' : assigned_to === 'loan_processor' ? 'Loan Processor' : 'Borrower'}): "${title}"`,
    })
  ...
  try {
    ...
    if (assigned_to === 'borrower') {
      ...
      // Specific-person pin overrides the role-wide blast.
...
export async function PUT(req: NextRequest) {
  ...
  try {
    await adminClient.from('loan_events').insert({
      ...
      description: `Loan processor ${lp.full_name} marked "${condition.title}" as ${status}${status === 'Rejected' && rejectionReason?.trim() ? `: ${rejectionReason.trim()}` : ''}`,
    })
  } catch (err) { console.error('Event log error:', err) }
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    await adminClient.from('loan_events').insert({
      ...
      description: `Loan processor ${lp.full_name} responded to "${condition.title}": ${response.trim()}`,
    })
  } catch (err) { console.error('Event log error:', err) }
