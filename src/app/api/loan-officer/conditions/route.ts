...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Inline borrower + LO names in every email so the recipient
    // knows which loan this is without clicking through.
...
export async function PUT(req: NextRequest) {
  ...
  // LO when View-As is active, else the logged-in LO.
  const supabase = await createClient()
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  ...
  const lo = await getEffectiveRoleRow<{ id: string; full_name: string }>(
  ...
  if (Array.isArray(mentions) && mentions.length > 0) {
    try {
      await processMentions({
        adminClient,
        authorName: lo.full_name ?? 'Loan Officer',
