// Server-rendered Committee Review Sheet PDF. Mirrors the Term
// Sheet endpoint pattern — fetches loan + details, renders via
// React-PDF, returns as a true downloadable file. Auth-gated to
// any staff role (admin / LO / LP / UW).

import { NextResponse } from 'next/server'
import { createClient } from '@/lib/supabase/server'
import { createAdminClient } from '@/lib/supabase/admin'
import { renderCommitteeReviewPdf } from '@/lib/pdf/committee-review-pdf'
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Letter date comes from the UI so the printed date matches what
  // the UW saw in the editable preview. Defaults to today server-side
  // if omitted.
  const url = new URL(req.url)
  const letterDate = url.searchParams.get('date') ?? undefined
  ...
  const pdf = await renderCommitteeReviewPdf({
    loan: {
  ...
  const safeFileSeed =
    (borrower?.full_name ?? loan.entity_name ?? loan.loan_number ?? id)
      ...
      .trim()
      .replace(/\s+/g, '_')
  const filename = `Committee_Review_${safeFileSeed || 'loan'}.pdf`
