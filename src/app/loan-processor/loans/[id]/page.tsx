...
export default async function LoanProcessorLoanPage({
  ...
  const borrower = loan.borrowers as { id: string; auth_user_id: string | null; full_name: string | null; email: string; phone: string | null } | null
    ...
    <PortalShell userName={lp.full_name} userRole="Loan Processor" dashboardHref="/loan-processor/inbox" variant="loan-processor" impersonation={isImpersonating ? {
      ...
      <Link href="/loan-processor/loans" className="text-sm text-primary hover:opacity-80 mb-4 inline-block">
        ...
        <div className="mt-2 mb-6 flex items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <UnclaimButton
              ...
              redirectTo="/loan-processor/loans"
              roleLabel="loan processor"
            />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-2 text-sm">
              ...
              <FieldRow label="Loan Number">
                <EditableLoanField loanId={loan.id} field="loan_number" type="text" currentValue={loan.loan_number} display={loan.loan_number ?? '—'} />
              </FieldRow>
              <FieldRow label="Loan Type">
                <EditableLoanField loanId={loan.id} field="loan_type" type="enum" options={LOAN_TYPES} currentValue={loan.loan_type} display={loan.loan_type ?? '—'} />
              </FieldRow>
              ...
              <FieldRow label="Rate Lock Date">
                <EditableLoanField loanId={loan.id} field="rate_lock_date" type="date" currentValue={loan.rate_lock_date} display={formatDate(loan.rate_lock_date)} />
              </FieldRow>
              ...
              <FieldRow label="Entity Name">
                <EditableLoanField loanId={loan.id} field="entity_name" type="text" currentValue={loan.entity_name} display={loan.entity_name ?? '—'} inputWidthClass="w-48" />
              </FieldRow>
              <FieldRow label="Funding Source">
                <EditableLoanField loanId={loan.id} field="funding_source" type="enum" options={['In House', 'RAI']} currentValue={loan.funding_source} display={loan.funding_source ?? '—'} />
              </FieldRow>
              <div className="flex justify-between items-center pt-2 mt-1 border-t border-gray-100">
                <span className="text-gray-500">Est. Closing Date</span>
                <EditableClosingDate loanId={loan.id} currentDate={loan.estimated_closing_date} />
              </div>
            </CardContent>
          </Card>
          ...
          />
          {/* Borrower Info — sits between Borrower Assignment and Co-borrowers */}
                ...
                <>
                  ...
                  {loanOfficer.title && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{loanOfficer.title}</span>
                    </div>
                  )}
                  {loanOfficer.email && (
                    <div className="flex justify-between">
                      <span className="text-gray-500">Email</span>
                      <a href={`mailto:${loanOfficer.email}`} className="font-medium text-primary hover:opacity-80">
                        {loanOfficer.email}
                      </a>
                    </div>
                  )}
                  {loanOfficer.phone && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">{loanOfficer.phone}</span>
                    </div>
                  )}
                </>
              ) : (
                <p className="text-gray-400 italic">No loan officer assigned</p>
              )}
            </CardContent>
          </Card>
        ...
        {/* Loan Processor Conditions */}
        <LoanProcessorConditions
          loanId={id}
