...
//   - Loans scoped to this user's assignment (LO/LP/UW), unless the
//     LP is an Operations Manager (`is_ops_manager=true`), in which
//     case we count across every active loan — same scope the LP
//     Inbox page uses for that user.
...
//             + if ops manager: also assigned_to=closer (Omayra wears
//               both hats — the email goes to her, the inbox should too)
...
export interface OutstandingCountOptions {
  /** Skip the ownership filter on the loan query — used for ops manager
   *  LPs who see the full pipeline. */
  allLoans?: boolean
  /** Also count conditions assigned to "closer". Used for ops manager
   *  LPs (Omayra) who are also the closer. */
...
export async function countOutstandingForRole(
  ...
  role: Role,
  roleId: string,
  options: OutstandingCountOptions = {},
): Promise<number> {
  const { allLoans = false, includeCloser = false } = options

  // 1. Find this user's active loans (or the whole pipeline for ops managers).
  ...
  for (const c of conditions ?? []) {
    const matchesRole = c.assigned_to === role
    const matchesCloser = includeCloser && c.assigned_to === 'closer'
    const yours =
      ((matchesRole || matchesCloser) && (c.status === 'Outstanding' || c.status === 'Rejected')) ||
